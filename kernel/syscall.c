...
syscall(void)
{
  ...
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {// 如果系统调用编号有效
    ...
    // if the mask equals to 1<<num(num is syscall num), then it returns non-zero
    // SYS_read = 5, 1<<5 = 32 = 10000=syscall_trace, so for all other num, 1<<num & syscall_trace will be 0
